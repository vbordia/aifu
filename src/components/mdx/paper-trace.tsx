...
export default function PaperTrace({
  nodes,
  edges,
  steps,
  height = 400,
  ...
  const edgeState = step.edges ?? {};

  const elements = [
    ...nodes.map((n) => ({
      data: { id: n.id, label: n.label ?? n.id, type: nodeState[n.id] ?? n.kind },
      position: { x: n.x, y: n.y },
    })),
    ...edges.map((e) => ({
      data: { id: e.id, source: e.from, target: e.to, label: e.label, type: edgeState[e.id] },
    })),
  ];
    ...
    <div className="my-6 overflow-hidden rounded-xl border border-foreground/[0.1] bg-foreground/[0.02]">
      <div className="flex flex-wrap items-center gap-2 border-b border-foreground/[0.08] px-3 py-2">
        <div className="flex gap-1.5">
          <button type="button" className={btn} disabled={active === 0} onClick={() => setActive((a) => Math.max(0, a - 1))}>
            ← Prev
          </button>
          <button
            type="button"
