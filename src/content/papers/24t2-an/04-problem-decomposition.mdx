...
OR(A) wins at 12, loses the marker when A's real cost (54) lands, regains it when C's refinement pushes AND(B,C) to 62 — the double flip is the whole exam point. The primary key 28,32,56 counts S after each of A, C, B; the alternate list 22,28,32 comes from a different counting convention in the official solution (the primary reading above reproduces every number with standard AO* arithmetic).
</Callout>
