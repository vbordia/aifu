...
Pop the cheapest partial path; ties break by path labels (SDE &lt; SDHF &lt; SAB).

| Pop | Extend to | OPEN afterwards (path : g) |
|-----|-----------|---------------------------|
| S : 0 | SA : 4, SD : 2 | SD : 2, SA : 4 |
| SD : 2 | SDE : 6, SDH : 4 | SA : 4, SDH : 4, SDE : 6 |
| SDH : 4 | SDHF : 6 | SA : 4, SDE : 6, SDHF : 6 |
| SA : 4 | SAB : 6, SAE : 12 | SDE : 6, SDHF : 6, SAB : 6, SAE : 12 |
| SDE : 6 | SDEC : 10, SDEH : 14 | SDHF : 6, SAB : 6, SDEC : 10, SAE : 12, SDEH : 14 |
| SDHF : 6 | SDHFG : 10 (goal on queue) | SAB : 6, SDHFG : 10, SDEC : 10, SAE : 12, SDEH : 14 |
| SAB : 6 | SABC : 10, SABG : **8** (goal on queue) | **SABG : 8**, SDHFG : 10, SABC : 10, SDEC : 10, SAE : 12, SDEH : 14 |
| **SABG : 8** | goal popped — cheapest open is 10 ≥ 8 | → **optimal** |
...
1. h-table first. A(7) vs D(5) sends Best First down the D branch; E and H tie at h = 5, so the label rule fires (E first).
2. BFS = sort by h (ties → labels); it never looks at edge costs — hence the 14 detour.
3. A* = sort by g+h; any node with f above the final goal cost is never expanded (E sits at f = 11 when G pops at 10).
4. BnB = sort by g on full paths; S,A,B,G = 8 pops before anything else can close.
