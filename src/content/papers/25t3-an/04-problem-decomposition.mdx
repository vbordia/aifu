...
**Expansion 3 — D** (marked path S→A→D,E; costliest branch D(2) > E(1)). D is **primitive** → instantly SOLVED. Nothing to revise: AND(D, E) stays $(2+2)+(1+2) = 7$, A stays 7 and **S stays 9** through this step — the paper still counts the selection as an expansion.
...
$$S = \min(16,\ 10+2) = \mathbf{12}$$ ✓ — S SOLVED
...
- **4.2** — S runs **10 → 9 → 9 (unchanged through D) → 12**: 10 when OR(A) is marked at S; 9 once A prices at 7; still 9 after the no-op D step; 12 when E's true cost (13) flips A onto I ($A = 10 \Rightarrow S = A + 2 = 12$). The official key writes the story as **`10,9,10,12`** — its third entry records A flipping to **10** during the final revision — while the alternate key **`9,10,12`** reports from the A-expansion onward. Both describe the same three distinct snapshots.
